...
std::string compensateZeros(int value){
    // The command position is expected to be three character. this function will compensate for command under three characters.
...
hardware_interface::return_type ArduinobotInterface::write(const rclcpp::Time & time,
    ...
    try{
        ...
        RCLCPP_ERROR_STREAM(rclcpp::get_logger("ARduinobotInterface"), "Something went wrong while sening the message " << msg << " to the port " << port_);
