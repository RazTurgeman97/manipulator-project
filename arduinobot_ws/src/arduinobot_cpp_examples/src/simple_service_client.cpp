...
public:
    SimpleServiceClient(int a, int b) : Node("simple_sevice_client"){
        ...
        while(!client_ -> wait_for_service(1s)){
            ...
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service is not avalible. waiting...");
            if(!rclcpp::ok()){
...
private:
    ...
    void responseCallback(rclcpp::Client<arduinobot_msgs::srv::AddTwoInts>::SharedFuture future){
        ...
        if(future.valid()){
            ...
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Serivce Faliure!");
        }
    }
};
...
int main(int argc, char* argv[]){
    ...
    if(argc!=3){
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Wrong number of arguments! Usage: simple_service_client <first_number> <second_number>");
        return 1;
    }
